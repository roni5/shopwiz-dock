...
/**
 * Add functionality to the cart panes tabledrag enhanced table.
 *
...
Drupal.behaviors.paneDrag = {
  attach: function (context, settings) {
    ...
    Drupal.theme.tableDragChangedWarning = function () {
      return '<div class="messages warning">' + Drupal.theme('tableDragChangedMarker') + ' ' + Drupal.t("Changes to the cart panes will not be saved until the <em>Save configuration</em> button is clicked.") + '</div>';
    };
    ...
    tableDrag.onDrop = function() {
      ...
      var pageField = $('select.cart-pane-page', dragObject.rowObject.element);

      if ($(dragObject.rowObject.element).prev('tr').is('.page-message')) {
        var weightField = $('select.cart-pane-weight', dragObject.rowObject.element);
        var oldPageName = weightField[0].className.replace(/([^ ]+[ ]+)*cart-pane-weight-([^ ]+)([ ]+[^ ]+)*/, '$2');

        if (!pageField.is('.cart-pane-page-'+ pageName)) {
          pageField.removeClass('cart-pane-page-' + oldPageName).addClass('cart-pane-page-' + pageName);
          weightField.removeClass('cart-pane-weight-' + oldPageName).addClass('cart-pane-weight-' + pageName);
    ...
    $('select.cart-pane-page', context).once('cart-pane-page', function () {
